...
void InitTimer1A(unsigned long period, unsigned long priority) {
  ...
  SYSCTL_RCGCTIMER_R |= 0x02;
	...
	TIMER1_CTL_R &= ~TIMER_CTL_TAEN; 														// 1) disable timer1A during setup: TIMER_CTL_TAEN is defined to be 0x00000001, this set BIT0 of the register to 0
  ...
  TIMER1_TAMR_R = TIMER_TAMR_TAMR_PERIOD; 										// 3) configure for periodic mode, default down-count settings
	TIMER1_TAILR_R = period - 1; 																// 4) reload value: Period is 1 minus the counter value          
...
int OS_AddPeriodicThread(void(*task)(void), unsigned long period, unsigned long priority) {
	PeriodicTask = task; // Assign task to the PeriodicTask
	InitTimer1A(period, priority); // Initialize Timer
	return 1;
}
